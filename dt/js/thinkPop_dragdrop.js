(() => {
  ...
  function setDropObj(__obj) {
    __obj.value = __obj.attr(ATTR_DRAG_OBJ).replace(/ /g, '').split(',');
  ...
  function setDragObj(__obj) {
    __obj.value = __obj.attr(ATTR_DRAG_OBJ);
  ...
  window.addEventListener('load', () => {
    ...
    if (container.attr(ATTR_OPTION)) {
      container.attr(ATTR_OPTION).replace(/ /g, '').split(',').forEach((option) => {
        DATA[option] = true;
      });
    }
    ...
    window.THINKPOP.dragdrop = dragdrop.reset;
  });
})();