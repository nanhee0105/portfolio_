window.addEventListener('load', () => {
    var starton = false;
    $('.dr_item').append('<span class="thinkagain">다시 풀어보세요</span>')
    ...
    function dragReset() {
      ...
      $('.dr_item').css('pointer-events','auto');
        $('.dr_item').removeClass('answerOn');
    }
    ...
    const dragEnd = function (event) {
      ...
      if (value == targetItem.dom.getAttribute('data-mark')) {
        ...
        effFail.play();
        $(targetItem.dom).find('.thinkagain').show();
        $('.dr_item').css('pointer-events','none');
        setTimeout(function(){
            targetItem.dom.style.left = ``;
            targetItem.dom.style.top = ``;
            $(targetItem.dom).find('.thinkagain').hide();
            $('.dr_item').css('pointer-events','auto');
        }, 1500);
      }
    ...
    $('.checkBtn').on('click', function(){
        if( starton ){
            ...
            $('.dr_item').css('pointer-events','none');
            $('.dr_item').addClass('answerOn');
        } else {
            $('.doitsolve').stop().fadeIn();
            setTimeout(function(){
                $('.doitsolve').stop().fadeOut();
            },1000);
        };
    });
  });